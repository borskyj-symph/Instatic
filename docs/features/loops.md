...
**Filtering by the entry being rendered.** `cellValue` accepts the same `{…}` tokens as any text binding, resolved per render against the page's `TemplateRenderDataContext`. This is what makes a filtered loop usable on an entry template: one course page lists the terms whose `course` cell matches `{currentEntry.slug}`, and the same template renders a different term list for every course. `resolveFilterTokens` in `server/publish/loopPrefetch.ts` does the substitution once, before the source fetches, so the sources and `cellFilterSql` never learn that tokens exist.

A token that resolves to nothing empties the loop instead of unfiltering it. `parseCellFilter` reads a blank `cellValue` as "not configured yet" and lists the whole table — correct for a loop mid-edit, wrong on a published entry route, where it would spill every other row onto the page. So a loop whose filter carried a token that resolved to nothing renders zero items. That also covers a page with no entry in scope at all: a `{currentEntry.*}` filter on an ordinary page, or inside a Layer C hole, renders empty.

The check is per token, not per string: `interpolateTokensWithStatus` reports whether any token failed, so `course-{currentEntry.missing}` — which interpolates to the non-blank `course-` and, with `isNot`, would match nearly the whole table — aborts too. A token whose `|fallback` fires counts as resolved: the author supplied that value. A malformed or unknown-source token is emitted verbatim as text, as everywhere else, and is not treated as a failure. The abort also respects the operator: `isTrue`, `isFalse`, `isSet` and `isEmpty` never read `cellValue` (`cellFilterUsesValue` in `cellFilter.ts`), so an unresolvable token sitting in a value those operators ignore leaves the loop querying normally.

A filter value containing `{route.query.*}` makes the loop node request-dependent, exactly like the same token in an ordinary string prop: `checkInlineTokens` in `dynamicDetection.ts` walks strings nested inside object props, so it sees `filters.cellValue`. The loop is then deferred to a Layer C hole, and the hole endpoint keys its cache on the originating page's full query — so `?course=A` and `?course=B` never share a slot. Only `route.query` is request-dependent; `{currentEntry.*}`, `{page.*}`, `{site.*}`, `route.path` and `route.slug` are all known at publish time and leave the loop in the baked page.

**A filter cannot mix `{route.query.*}` with `{currentEntry.*}`.** The query token defers the loop to a hole, and a hole is fetched by node id with no entry in scope — `renderHoleFragment` seeds an empty `entryStack` — so the entry half of the filter resolves to nothing and the loop renders empty. `"{currentEntry.slug}-{route.query.region}"` on a course page is the shape that hits this. Filtering an entry template's loop by a query parameter needs the hole endpoint to resolve the entry from the request path first, which it does not do today.

**A nested loop cannot filter on the loop around it.** `prefetchLoopData` resolves every loop node once, keyed by node id, against ONE shared context — the per-iteration entry stack is built later, by the renderer, and the prefetch never sees it. So inside an outer loop, a `{currentEntry.*}` filter on the inner loop reads the page's entry (the row the template is rendering), not the outer loop's current item. Every iteration of the outer loop therefore gets the same inner list. Filtering an inner list per outer item needs a different shape — e.g. an entry-field loop over a relation cell.

**What the author sees while editing.** Three surfaces behave differently, and the difference is the entry each one has in scope:

- *The canvas itself* sends the filter value to the loop-preview endpoint verbatim — `useLoopPreviewItems` reads `cellValue` as a raw string and never interpolates. The literal token matches nothing, so the canvas falls back to synthetic preview items and the loop body stays visible while it is laid out.
- *Preview (and the explorer hover preview)* posts the canvas's `templateContext` to the runtime preview endpoint, which forwards it into the prefetch — so filters resolve there. On a post-type template that context carries a REAL row (the first published one, or the one picked in `TemplateModeControl`), and the preview shows that row's real matches. Only when the table has no published rows at all does the context fall back to a synthetic sample row, whose values match nothing and render the loop empty.
- *The row preview in the Content workspace* has the edited row as its entry, and filters resolve exactly as on the published page.
