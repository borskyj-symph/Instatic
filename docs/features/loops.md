...
A token that resolves to nothing empties the loop instead of unfiltering it. `parseCellFilter` reads a blank `cellValue` as "not configured yet" and lists the whole table — correct for a loop mid-edit, wrong on a published entry route, where it would spill every other row onto the page. So a loop whose filter carried tokens and resolved blank renders zero items. That also covers a page with no entry in scope at all: a `{currentEntry.*}` filter on an ordinary page, or inside a Layer C hole, renders empty.

On the canvas the token is not resolved — the author is editing the template, not a row, so there is no current entry. The loop-preview endpoint receives the literal token, matches nothing, and the preview falls back to a synthetic item so the loop body stays visible while it is laid out. The row preview in the Content workspace does have an entry, and filters there resolve exactly as on the published page.
