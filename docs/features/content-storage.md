...
The published `SiteDocument` is stored ONCE per full publish in `site_snapshots` (with a content hash used by the publish-status check); each page's `data_row_versions` row references it via `site_snapshot_id` and carries only its page-scoped `runtime_assets_json`. Readers reassemble the `PublishedPageSnapshot` from the join — that snapshot is the canonical audit record. Each full publish also stores a fresh copy of every page's runtime scripts in `published_runtime_assets`, so without pruning the database grows by the whole site on every publish. `prunePublishHistory` (same transaction) keeps the newest `PUBLISH_HISTORY_KEEP` (20) snapshots plus any snapshot an active version still references, and deletes older snapshots and the runtime scripts of inactive versions outside them. The `data_row_versions` rows stay, so version history is complete; a pruned version only loses its `site_snapshot_id`. After it's written, the publisher routes through the three-layer pipeline:
